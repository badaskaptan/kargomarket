    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        SET pickup_date_preferred = COALESCE(
          ...
          CASE 
            ...
            THEN (proposed_dates->>'pickup_date')::timestamp with time zone 
            ELSE NULL 
          END
        ),
        delivery_date_preferred = COALESCE(
          ...
          CASE 
            ...
            THEN (proposed_dates->>'delivery_date')::timestamp with time zone 
            ELSE NULL 
          END
        )
...
COMMENT ON COLUMN public.service_offers.message IS 'Ana teklif mesajı (eski service_description verilerini içerir)';
COMMENT ON COLUMN public.service_offers.expires_at IS 'Teklifin geçerlilik süresi (eski validity_period ve valid_until birleştirildi)';
COMMENT ON COLUMN public.service_offers.pickup_date_preferred IS 'Tercih edilen alım tarihi (eski proposed_dates migration)';
COMMENT ON COLUMN public.service_offers.delivery_date_preferred IS 'Tercih edilen teslimat tarihi (eski proposed_dates migration)';
